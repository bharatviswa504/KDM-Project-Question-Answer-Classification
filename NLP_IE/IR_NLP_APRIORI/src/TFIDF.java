...
public class TFIDF {
	...
	public void main() throws IOException, JATEException {
		boolean a = true;
		if (!a) {
		System.out.println("Usage: java TestTfIdf [path_to_corpus] [output_folder]");
		} else {
			System.out.println("Started TFIDF"+"at: " + new Date() + "... For detailed progress see log file jate.log.");
			...
			GlobalIndexMem termDocIndex = builder.build(new CorpusImpl("input1"), npextractor);
			...
			tester.execute(termDocIndex, "output1");
